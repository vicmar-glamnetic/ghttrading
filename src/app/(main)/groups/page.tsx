...
function GroupCard({ group, onJoin, joining }: { group: GroupSummary; onJoin: () => void; joining: boolean }) {
  ...
  return (
    <div className="bg-surface rounded-xl border border-line overflow-hidden hover:border-line2 transition-colors">
      {/* Cover placeholder */}
      <div className="h-20 bg-linear-to-br from-yellow-500/10 to-elevated flex items-center justify-center">
        {group.image ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={group.image} alt={group.name} className="w-14 h-14 rounded-full object-cover border-2 border-surface" />
        ) : (
          <div className="w-14 h-14 rounded-full bg-yellow-500/20 border-2 border-surface flex items-center justify-center">
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-2 mb-1">
          <Link href={`/groups/${group.id}`} className="font-semibold text-ink hover:text-yellow-500 transition-colors text-sm leading-tight">
            {group.name}
          </Link>
          <span className={`flex items-center gap-1 text-[10px] shrink-0 px-1.5 py-0.5 rounded-full border ${group.privacy === 'private' ? 'border-line2 text-ink3' : 'border-yellow-500/20 text-yellow-600'}`}>
            {group.privacy === 'private' ? <Lock className="w-2.5 h-2.5" /> : <Globe className="w-2.5 h-2.5" />}
            {group.privacy}
          </span>
        </div>
        {group.description && <p className="text-xs text-ink3 line-clamp-2 mb-3">{group.description}</p>}
        <div className="flex items-center justify-between">
          <p className="text-[10px] text-line2">{group._count.members} member{group._count.members !== 1 ? 's' : ''} · {group._count.posts} post{group._count.posts !== 1 ? 's' : ''}</p>
          {isMember ? (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-surface rounded-2xl border border-line p-6 w-full max-w-md shadow-2xl">
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-bold text-ink">Create Group</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="text-xs font-semibold text-ink2 uppercase tracking-wider block mb-1.5">Group Name *</label>
            <input
              value={name}
          ...
          <div>
            <label className="text-xs font-semibold text-ink2 uppercase tracking-wider block mb-1.5">Privacy</label>
            <div className="grid grid-cols-2 gap-2">
              {(['public', 'private'] as const).map(p => (
                <button
                  ...
                  className={`flex items-center gap-2 p-3 rounded-lg border text-left transition-colors ${privacy === p ? 'border-yellow-500/50 bg-yellow-500/5 text-yellow-400' : 'border-line text-ink3 hover:border-line2'}`}
                >
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="font-bold text-ink text-lg">Groups</h1>
        </div>
          ...
          {loading ? (
            ...
            !data?.myGroups.length ? (
              <div className="py-12 text-center">
                ...
                <p className="text-ink3 text-sm">You haven&apos;t joined any groups yet</p>
            ...
            !data?.discover.length ? (
              <div className="py-12 text-center">
                <Globe className="w-10 h-10 text-line mx-auto mb-3" />
                <p className="text-ink3 text-sm">No public groups yet</p>
