...
export async function GET() {
  ...
  const entries = await db.journalEntry.findMany({
    ...
    select: { authorId: true, pnl: true, result: true },
  })

  const agg = new Map<string, { pnl: number; wins: number; losses: number; trades: number }>()
  for (const e of entries) {
    const a = agg.get(e.authorId) ?? { pnl: 0, wins: 0, losses: 0, trades: 0 }
  ...
  const rows = users
    .map(u => {
      const a = agg.get(u.id) ?? { pnl: 0, wins: 0, losses: 0, trades: 0 }
